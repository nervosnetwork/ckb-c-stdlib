...
MOLECULE_API_DECORATOR void mol_builder_initialize_fixed_size(
    mol_builder_t *builder, mol_num_t fixed_size) {
  if (fixed_size == 0) {
...
MOLECULE_API_DECORATOR void mol_union_builder_initialize(
...
MOLECULE_API_DECORATOR void mol_builder_initialize_with_capacity(
...
MOLECULE_API_DECORATOR void mol_fixvec_builder_initialize(
    mol_builder_t *builder, mol_num_t data_capacity) {
...
MOLECULE_API_DECORATOR void mol_table_builder_initialize(
    mol_builder_t *builder, mol_num_t data_capacity, mol_num_t field_count) {
  mol_builder_initialize_with_capacity(builder, data_capacity,
                                       MOL_NUM_T_SIZE * field_count * 2);
  memset(builder->number_ptr, 0x00, builder->number_cap);
  builder->number_used = builder->number_cap;
}

MOLECULE_API_DECORATOR void mol_option_builder_set(mol_builder_t *builder,
  ...
  if (builder->data_used == 0) {
    ...
    if (builder->data_cap < builder->data_used) {
      ...
      builder->data_ptr =
          (uint8_t *)realloc(builder->data_ptr, builder->data_cap);
    }
    memcpy(builder->data_ptr, data_ptr, builder->data_used);
  }
}

MOLECULE_API_DECORATOR void mol_union_builder_set_byte(mol_builder_t *builder,
                                                       mol_num_t item_id,
                                                       uint8_t data) {
...
MOLECULE_API_DECORATOR void mol_union_builder_set(mol_builder_t *builder,
                                                  mol_num_t item_id,
                                                  const uint8_t *data_ptr,
                                                  mol_num_t data_len) {
...
MOLECULE_API_DECORATOR void mol_builder_set_byte_by_offset(
    mol_builder_t *builder, mol_num_t offset, uint8_t data) {
  *(builder->data_ptr + offset) = data;
}

MOLECULE_API_DECORATOR void mol_builder_set_by_offset(mol_builder_t *builder,
                                                      mol_num_t offset,
                                                      const uint8_t *data_ptr,
                                                      mol_num_t length) {
...
MOLECULE_API_DECORATOR void mol_fixvec_builder_push_byte(mol_builder_t *builder,
                                                         uint8_t data) {
...
MOLECULE_API_DECORATOR void mol_fixvec_builder_push(mol_builder_t *builder,
                                                    const uint8_t *data_ptr,
                                                    mol_num_t length) {
...
MOLECULE_API_DECORATOR void mol_dynvec_builder_push(mol_builder_t *builder,
                                                    const uint8_t *data_ptr,
                                                    mol_num_t data_len) {
  ...
  while (builder->number_cap < builder->number_used + MOL_NUM_T_SIZE) {
    ...
    builder->number_ptr =
        (mol_num_t *)realloc(builder->number_ptr, builder->number_cap);
  }
...
MOLECULE_API_DECORATOR void mol_table_builder_add_byte(mol_builder_t *builder,
...
MOLECULE_API_DECORATOR void mol_table_builder_add(mol_builder_t *builder,
                                                  mol_num_t field_index,
                                                  const uint8_t *data_ptr,
                                                  mol_num_t data_len) {
  if (data_len == 0) {
...
mol_builder_finalize_simple(mol_builder_t builder) {
  mol_seg_res_t res;
  res.errno = MOL_OK;
...
MOLECULE_API_DECORATOR mol_seg_res_t
mol_fixvec_builder_finalize(mol_builder_t builder) {
  mol_seg_res_t res;
  res.errno = MOL_OK;
...
MOLECULE_API_DECORATOR mol_seg_res_t
mol_dynvec_builder_finalize(mol_builder_t builder) {
  mol_seg_res_t res;
  res.errno = MOL_OK;
  ...
  for (mol_num_t number_index = 0; number_index < number_count;
       number_index++) {
    builder.number_ptr[number_index] += header_size;
  }
  memcpy((res.seg.ptr + MOL_NUM_T_SIZE), builder.number_ptr,
         builder.number_used);
  memcpy((res.seg.ptr + MOL_NUM_T_SIZE + builder.number_used), builder.data_ptr,
         builder.data_used);
  mol_builder_discard(builder);
  return res;
}
